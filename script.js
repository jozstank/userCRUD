...
const dataFromServer = async () => {
  const fetchData = await fetch("https://node-project-fikx.onrender.com/users");
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const fetchForDelete = async () => {
      const fet = await fetch("https://node-project-fikx.onrender.com/users", {
        method: "DELETE",
        body: JSON.stringify({
...
const registerUser = async () => {
  const response = await fetch("https://node-project-fikx.onrender.com/users", {
    method: "POST",
...
btn.addEventListener("click", () => {
  ...
  const fileuplaod = async () => {
    const fetchForFileupload = await fetch(
      "https://node-project-fikx.onrender.com/fileupload",
      {
        method: "POST",
        body: input.files[0],
      }
  ...
  const fetchForUpdate = async () => {
    const fetcH = await fetch("https://node-project-fikx.onrender.com/users");
